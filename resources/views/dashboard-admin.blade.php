    ...
    <div class="grid grid-cols-3 gap-20 ml-64">
        ...
        <div class="bg-white rounded shadow text-center grid grid-cols-2">
            ...
            <div class="flex flex-col justify-center bg-biru">
                ...
                <p id="konsultasiCount" class="text-3xl text-white font-bold">{{ $konsultasiCount }}</p>
            </div>
        </div>
        <!-- Card 2 -->
        <div class="bg-white rounded shadow text-center grid grid-cols-2">
            ...
            <div class="flex flex-col justify-center bg-hijau">
                ...
                <p id="permintaanDataCount" class="text-3xl text-white font-bold">{{ $permintaanDataCount }}</p>
            </div>
        </div>
        <!-- Card 3 -->
        <div class="bg-white rounded shadow text-center grid grid-cols-2">
            ...
            <div class="flex flex-col justify-center bg-orange">
                ...
                <p id="lainnyaCount" class="text-3xl text-white font-bold">{{ $lainnyaCount }}</p>
            </div>
        </div>
    </div>
    ...
    <script>
        function updateCounts() {
            fetch("{{ route('dashboard.counts') }}")
                ...
                .then(data => {
                    document.getElementById('konsultasiCount').innerText = data.konsultasiCount;
                    document.getElementById('permintaanDataCount').innerText = data.permintaanDataCount;
                    document.getElementById('lainnyaCount').innerText = data.lainnyaCount;
                });
        }

        setInterval(updateCounts, 5000); // Update setiap 5 detik
    </script>
</x-layout-admin>
